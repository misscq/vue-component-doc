<template>
  <section class="prev-next-nav">
    <router-link v-if="prev" key="prev" class="prev-page" :to="prev.path">
      <span
        role="img"
        aria-label="left"
    ...
    <router-link v-if="next" key="next" class="next-page" :to="next.path">
      <span>{{ next.title }}</span>
      <span v-if="isZhCN" class="chinese">{{ next.subtitle }}</span>
      <span
        role="img"
        aria-label="right"
        class="anticon anticon-right footer-nav-icon-after"
      >
        <svg
          viewBox="64 64 896 896"
          focusable="false"
